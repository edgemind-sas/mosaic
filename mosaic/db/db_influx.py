...
class DBInfluxDB(DBBase):
    def connect(self, **params):
        bucket = self.name if self.name else self.endpoint.split('/')[0]
        measurement = self.endpoint.split('/')[-1]
        self.bkd = InfluxDBClient(url=self.config.url, token=self.config.token, org="", **params)
    ...
    def count(self, filter={}, **params):
        query = f'from(bucket: "{self.name}") |> range(start: 0) |> filter(fn: (r) => r._measurement == "{self.endpoint}"'
        if filter:
            query += f' and {filter}'
        query += ') |> count()'

        try:
            result = self.bkd.query_api().query_data(query=query)
            count = result[0].records[0].values["_value"]
            return count
        except Exception as e:
            if self.logger:
                self.logger.error(f"Failed to retrieve data count from InfluxDB: {str(e)}")
            return 0

    def get(self, filter={}, projection=[], limit=0, keep_influx_id=False, **params):
        query = f'from(bucket: "{self.name}") |> range(start: 0) |> filter(fn: (r) => r._measurement == "{self.endpoint}"'
        if filter:
            query += f' and {filter}'
        query += ')'
    ...
    def put(self, data=[], index=[], **params):
        write_api = self.bkd.write_api(write_options=SYNCHRONOUS)

        try:
            for item in data:
                point = Point(self.endpoint)
                for tag in index:
                    point.tag(tag, item[tag])
                for key, value in item.items():
                    if key not in index:
                        point.field(key, value)
                write_api.write(bucket=self.name, record=point)
    ...
    def update(self, data=[], index=[], set_on_insert_data={}, **params):
        write_api = self.bkd.write_api(write_options=SYNCHRONOUS)
        ...
        try:
            for item in data:
                ...
                for key, value in item.items():
                    if key not in index:
                        point.field(key, value)
                point = point.set(**set_on_insert_data)
                write_api.write(bucket=self.name, record=point)
            return {"success_count": len(data), "failure_count": 0}
        except Exception as e:
            if self.logger:
                self.logger.error(f"Failed to update data in InfluxDB: {str(e)}")
            return {"success_count": 0, "failure_count": len(data)}
    ...
    def delete(self, filter={}, **params):
        ...
        try:
            self.bkd.query_api().query(query)
            return {"success_count": 1, "failure_count": 0}
        except Exception as e:
            if self.logger:
                self.logger.error(f"Failed to delete data from InfluxDB: {str(e)}")
            return {"success_count": 0, "failure_count": 1}

    def reset(self, **params):
        try:
            self.bkd.buckets_api().delete_bucket(bucket=self.name)
            if self.logger:
                self.logger.info(f"InfluxDB database '{self.name}' has been reset")
            return True
        except Exception as e:
            if self.logger:
                self.logger.error(f"Failed to reset InfluxDB: {str(e)}")
            return False"""
    ...
    """def put(self, endpoint, data=[], index=[], time_field="_time", **params):
        ...
        try:
            ...
            for idx in index:  # tags
                if idx in item:
                    point.tag(idx, item[idx])

        # Traiter chaque clé/valeur dans item comme un champ, sauf si c'est dans index, "measurement" ou "time_field"
            for key, value in item.items():
